...
class SequentialNav(Node):
    def __init__(self):
        ...
        self._cancellation_service_1 = self.create_service(Empty, '/cancellation_1', self.handle_cancellation_1)
        self._cancellation_service_2 = self.create_service(Empty, '/cancellation_2', self.handle_cancellation_2)
        self._cancellation_service_3 = self.create_service(Empty, '/cancellation_3', self.handle_cancellation_3)
    ...
    def goal_response_callback(self, future):
        self.goal_handle1 = future.result()
        if not self.goal_handle1.accepted:
            self.send_next_goal()
            return

        self.get_logger().info('Goal accepted.')
        self._get_result_future = self.goal_handle1.get_result_async()
    ...
    def handle_cancellation_1(self,request,response):
        table_position = self.table_positions.get(1)
        self.goals = [goal for goal in self.goals if not (goal.pose.position.x == table_position[0] and goal.pose.position.y == table_position[1])]
        if self.current_goal_index == 1:
            self.confirmation_timer = None
            self._action_client._cancel_goal_async(self.goal_handle1)
        self.get_logger().info('table 1 cancel the order')
        return response
    def handle_cancellation_2(self,request,response):
        table_position = self.table_positions.get(2)
        self.goals = [goal for goal in self.goals if not (goal.pose.position.x == table_position[0] and goal.pose.position.y == table_position[1])]
        if self.current_goal_index == 1 or self.current_goal_index == 2:
            self.confirmation_timer = None
            self._action_client._cancel_goal_async(self.goal_handle1)

        self.get_logger().info('table 2 cancel the order')
        return response
    def handle_cancellation_3(self,request,response):
        table_position = self.table_positions.get(3)
        self.goals = [goal for goal in self.goals if not (goal.pose.position.x == table_position[0] and goal.pose.position.y == table_position[1])]
        if self.current_goal_index == 1 or self.current_goal_index == 2 or self.current_goal_index == 3:
            self.confirmation_timer = None
            self._action_client._cancel_goal_async(self.goal_handle1)
        self.get_logger().info('table 3 cancel the order')
        return response
